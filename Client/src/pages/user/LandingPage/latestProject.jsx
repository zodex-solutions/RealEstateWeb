...
//       statuses: property.statuses || "No Status",
...
//       } else {
//         dynamicOptions = [
//           ...new Set(allImages.map((item) => item[key]).filter(Boolean)),
//         ];
//       }
//       // else if (key === "TTILE") {
//       //   const allLocations = locationData.map((location) => location.location);
...
//       };
//     });
//   }, [propertyType, propertyStatus, locationData, allImages]);
...
//       if (!value) {
//         return true; // If no value is selected for this filter, include all images
//       }
//       if (key === "BEDS") {
//         const bedCount = value.split(" ")[0]; // Extract the number part from "2 Beds", "3 Beds", etc.
//         return image.beds === bedCount; // Compare the number of beds
//       }

//       // Handle the STATUS filter: match "Off-Plan" to image.statuses
//       if (key === "STATUS") {
//         return (
//           image.statuses && image.statuses.toLowerCase() === value.toLowerCase()
//         );
//       }

//       // Handle the LOCATION filter: match "JVC" to image.location
//       if (key === "LOCATION") {
//         return (
...
//       // Handle the TYPE filter: match "APARTMENT" to image.types
//       if (key === "TYPE") {
//         return image.types && image.types.toLowerCase() === value.toLowerCase();
//       }
...
//         {filteredImages
//           .filter((item) => !item.image.endsWith(".mp4"))
//           .slice(0, 8)
//           .map((item, index) => (
//             <div
//               onClick={() => {
...
export default function LatestProjects({ allPropertiesData }) {
  ...
  // );
  // Filter images based on selected filters
  const filteredImages = allPropertiesData.filter((image) =>
    Object.entries(filters).every(([key, value]) => {
      if (!value) {
        return true;
      }
